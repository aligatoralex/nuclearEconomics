...
def _generate_new_results(registry, scenarios) -> None:
    ...
    step7_tornado_*.csv/step8_sobol_*.csv paths, which stay untouched as
    the Krok 7/8 "before" reference (already reviewed/committed).
    """
    ...
    for tech_key, _label in TECHNOLOGIES:
        ...
        for wacc_kind in ("government", "commercial"):
            ...
            main_effects, _interactions = run_sobol_analysis(registry, scenario, sobol_params)
...
def _old_vs_new(old_df: pd.DataFrame, new_df: pd.DataFrame, value_col: str) -> pd.DataFrame:
...
def _comparison_traces(comparison: pd.DataFrame) -> list[go.Bar]:
    ...
    return [
        go.Bar(y=comparison.index, x=comparison["old"], orientation="h", name="stary (Krok 7-8)", marker_color=OLD_COLOR, showlegend=False),
        go.Bar(y=comparison.index, x=comparison["new"], orientation="h", name="nowy (Krok 9c)", marker_color=NEW_COLOR, showlegend=False),
    ]
...
def _build_comparison_figure() -> go.Figure:
    fig = make_subplots(
        ...
        cols=2,
        subplot_titles=[
            "Tornado range - AP1000", "Tornado range - CANDU EC6",
            "Sobol ST - AP1000", "Sobol ST - CANDU EC6",
        ],
        vertical_spacing=0.15,
    )

    for col, (tech_key, _label) in enumerate(TECHNOLOGIES, start=1):
        old_tornado = pd.read_csv(DATA_DIR / f"step7_tornado_{tech_key}.csv")
        ...
        old_sobol = pd.read_csv(DATA_DIR / f"step8_sobol_{tech_key}.csv")
    ...
    fig.add_trace(go.Bar(x=[None], y=[None], marker_color=OLD_COLOR, name="stary (Krok 7-8, CANDU CAPEX niedoszacowany)", showlegend=True))
    fig.add_trace(go.Bar(x=[None], y=[None], marker_color=NEW_COLOR, name="nowy (Krok 9c, CAPEX porownywalny)", showlegend=True))
