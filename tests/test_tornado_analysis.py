...
def test_wacc_ranks_in_top_3_for_all_scenarios():
    ...
    scenarios = {
        ("ap1000", "government"): AP1000_GOVERNMENT,
        ("ap1000", "commercial"): AP1000_COMMERCIAL,
        ("candu_ec6", "government"): CANDU_GOVERNMENT,
        ("candu_ec6", "commercial"): CANDU_COMMERCIAL,
    }
    ...
    for (tech, wacc_kind), scenario in scenarios.items():
        df = run_oat_tornado(registry, scenario)
        top3 = df.head(3)["parameter"].tolist()
        assert scenario.wacc_parameter_name in top3, (tech, wacc_kind, df)
